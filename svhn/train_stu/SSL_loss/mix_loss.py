...
import numpy as np
from utils import mixmatch_interleave
...
class MixMatchLoss(nn.Module):
    ...
    def cal_loss(self, logit_x, y, logit_u_x, y_hat, epoch, lambda_u):
        ...
        linear_weight = float(
            np.clip(epoch / self.epochs, 0.0, 1.0))  # linearly ramp up the contribution of unlabeled set
    ...
    def forward(self, input):
        ...
        loss_x, loss_u, w = self.cal_loss(logits_x, mixed_target[:self.bt], logits_u, mixed_target[self.bt:], current,
                                          self.lambda_u)

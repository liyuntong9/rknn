...
class BaseModel(ABC):
    ...
    def _save_checkpoint(self, epoch, is_best_val=False, ema_is_best_val=False):
        ...
        if ema_is_best_val:
            torch.save(checkpoint, os.path.join(self.out_dir, 'ema_best.pth'))
            self.logger.info("  EMA Best saving ... ")
        else:
            torch.save(checkpoint, os.path.join(self.out_dir, 'ema_latest.pth'))
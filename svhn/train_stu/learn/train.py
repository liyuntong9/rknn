...
from utils import get_traindata, AverageMeter, get_mixmatch_transform, accuracy
from .base import BaseModel
from SSL_loss import MixMatchLoss
from tqdm import tqdm
from model import WRN, WeightEMA


class Trainer(BaseModel):
    def __init__(self, configs, labeled_idx_file, label_file):
        ...
        transform_train, transform_val = get_mixmatch_transform(configs.dataset)

        train_labeled_set, train_unlabeled_set, val_set = get_traindata(configs.datapath,
                                                                            configs.dataset, configs.K,
                                                                            labeled_idx_file=labeled_idx_file,
                                                                            label_file=label_file,
                                                                            transform_train=transform_train,
                                                                            transform_val=transform_val)

        if configs.method == 'Mixmatch':
            self.train_loader = data.DataLoader(train_labeled_set, batch_size=configs.batch_size, shuffle=True,
                                                num_workers=0, drop_last=True)
            self.u_train_loader = data.DataLoader(train_unlabeled_set, batch_size=configs.batch_size, shuffle=True,
                                                  num_workers=0, drop_last=True)
        else:
        ...
        self.ema_optimizer = WeightEMA(self.model, self.ema_model, configs.weight_decay * configs.lr,
                                       alpha=configs.ema_alpha)
    ...
    def evaluate(self, epoch):
        ...
        with torch.no_grad():
            ...
            for x, y in tq:
                ...
                if self.configs.verbose:
                    tq.set_description(
                        "[{}] Epoch:{}. Top1: {:.4f}. Top5: {:.4f}. Loss: {:.4f}.".format("VAL", epoch, top1_meter.avg,
                                                                                          top5_meter.avg,
                                                                                          loss_meter.avg))
        self.logger.info(
            "  [{}] Epoch:{}. Top1: {:.4f}. Top5: {:.4f}. Loss: {:.4f}.".format(" VAL ", epoch, top1_meter.avg,
                                                                                top5_meter.avg, loss_meter.avg))
        self.logger.info(
            "  [{}] Epoch:{}. Top1: {:.4f}. Top5: {:.4f}. Loss: {:.4f}.".format(" EMA ", epoch, top1_ema_meter.avg,
                                                                                top5_ema_meter.avg, loss_ema_meter.avg))
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.configs.epochs):
            ...
            for it in tq:
                ...
                if self.configs.verbose:
                    tq.set_description(
                        " Epoch [{}/{}], iter: {}. loss: {:.4f}. loss_x: {:.4f}. loss_u: {:.4f}.  weight :{:.4f} Time: {:.2f}".format(
